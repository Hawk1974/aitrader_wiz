---
name: ai_trader_health_check
description: "Validates environment, folders, schemas, credentials presence, emergency state, and report writability. Does not make trade decisions."
platforms: [windows]
metadata:
  hermes:
    tags: [ai-trader, alpaca, paper-trading, risk, audit]
---
# Skill: ai_trader_health_check

## Classification

Python-backed deterministic health check.

## Purpose

Validates environment, folders, schemas, credentials presence, emergency state, and report writability. Does not make trade decisions.
...
- Script: `scripts/validation/ai_trader_health_check.py`
- Required CLI args: `--check all|config|credentials|folders|schemas|state|broker|reports`
- Required schemas/artifacts: `health_report, script_result_envelope`
