---
name: manual_approval_queue
description: "Manages approval artifacts required for manual modes. Expired/wrong-hash approval fails closed."
platforms: [windows]
metadata:
  hermes:
    tags: [ai-trader, alpaca, paper-trading, risk, audit]
---
# Skill: manual_approval_queue

## Classification

Python-backed deterministic approval queue.

## Purpose

Manages approval artifacts required for manual modes. Expired/wrong-hash approval fails closed.
...
- Script: `scripts/trading/manual_approval_queue.py`
- Required CLI args: `--action create|approve|reject|expire|status --order-intent-hash <hash> --actor <identity> --reason <text>`
- Required schemas/artifacts: `manual_approval, script_result_envelope`
